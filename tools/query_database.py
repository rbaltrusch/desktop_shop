...
def query_power_users(cursor):
    ...
    """
    data = cursor.execute("""SELECT l.first_name, l.last_name, l.dob, COUNT(*), SUM(r.cost)
...
def query_transactions_for_user(cursor, email):
    ...
    data = cursor.execute("""SELECT * FROM transactions WHERE user_id IN
...
def query_transactions_for_user_by_pattern(cursor, pattern):
    ...
    data = cursor.execute("""SELECT user_id FROM users
                          WHERE last_name LIKE ? ORDER BY first_name""", (pattern,))
    data = list(data)
    if not data:
        return
...
def query_transaction_price(cursor, transaction_id):
    ...
    data = cursor.execute("""SELECT SUM(price) FROM products
                          WHERE product_id in
...
def list_cheapest_transactions(cursor, limit=10):
    ...
    data = cursor.execute("""SELECT date, cost, user_id FROM transactions
                          ORDER BY cost ASC LIMIT ?""", (limit, ))
    for row in data:
        print(row)

def list_biggest_consumers(cursor, orders=20):
    """Lists the users that have more than the specified amount of orders"""
    data = cursor.execute("""SELECT user_id, COUNT(*), SUM(cost) FROM transactions
                          GROUP BY user_id HAVING COUNT(*) > ?
                          ORDER BY SUM(cost) DESC""", (orders,))
    for row in data:
        print(row)
