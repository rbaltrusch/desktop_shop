...
def init_main_menu_view(window):
    ...
    #home button
    home_button = tk.Button(main_menu_frame, command=callbacks.switch_to_home, text='Home', fg='black')
    component = Component(home_button, row=0, column=0)
    main_menu_view.add_component(component, 'home_button')

    #login button
    login_button = tk.Button(main_menu_frame, command=callbacks.switch_to_login, text='Login', fg='black')
    component = Component(login_button, row=0, column=2, sticky='nsew')
    main_menu_view.add_component(component, 'login_button')

    #register button
    register_button = tk.Button(main_menu_frame, command=callbacks.switch_to_register, text='Register', fg='black')
    component = Component(register_button, row=0, column=1)
    main_menu_view.add_component(component, 'register_button')

    #checkout button
    checkout_button = tk.Button(main_menu_frame, command=callbacks.switch_to_checkout, text='Checkout', fg='black')
...
def init_login_view(window):
    ...
    #login button
    login_button = tk.Button(login_frame, text='Log in', command=callbacks.login)
...
def init_register_view(window):
    ...
    #register button
    register_button = tk.Button(register_frame, text='Register', command=callbacks.register)
...
def init_product_data_in_home_view():
    ...
    for i, (product_id, name, price) in enumerate(product_datas):
        ...
        product_frame = tk.Frame(root, relief=tk.RAISED, bd=3)
        ...
        add_to_cart_button = tk.Button(product_frame, text='Add to cart', command=callbacks.add_to_cart, bg='lightgreen', bd=3, name=component_name)
...
def init_checkout_data_in_checkout_view():
    ...
    for i, (product_id, name, price) in enumerate(data_packets, 1):
        ...
        add_to_cart_button = tk.Button(product_frame, text='Remove from cart', command=callbacks.remove_from_cart, bg='orange', bd=3, name=component_name)
        component = Component(add_to_cart_button, row=row_num, column=2, sticky='e')
        checkout_view.add_component(component, component_name)

    row_num += 1
    #confirm transaction button
    confirm_button = tk.Button(root, text='Confirm transaction', command=callbacks.place_order, bg='lightgreen', bd=3, name=component_name)
...
def init_profile_view(window):
    ...
    edit_user_data_button = tk.Button(profile_data_frame, text='Confirm changes', bg='lightgreen', command=callbacks.edit_user_data, bd=3)
    ...
    password_change_frame_button = tk.Button(password_frame, text='Change password', command=callbacks.show_password_change_frame, bd=3)
    ...
    password_change_button = tk.Button(password_change_frame, text='Confirm password change', command=callbacks.edit_user_password, bd=3)
